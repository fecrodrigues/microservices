...
export default {
    async created() {
        ...
        let rawResponse = await fetch(process.env.VUE_APP_BACKEND_API + '/api/v1/users/' + userID);
        let rawResponseList = await fetch(process.env.VUE_APP_BACKEND_API + '/api/v1/users?limit=999&advocate=true');
